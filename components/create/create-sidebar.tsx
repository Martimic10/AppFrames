...
export function CreateSidebar({
  ...
  const currentGraphicRotate = resolvedGraphicPosition.rotate ?? 0;
  ...
  const applyGraphicRotate = (rotate: number) => {
    onSelectedSlideStyleChange({
      graphicPosition: clampGraphicPosition({
        ...resolvedGraphicPosition,
        rotate
      })
    });
            ...
            >
              ...
              <div className="mb-3 rounded-xl border border-white/10 bg-zinc-900/45 p-2.5">
                ...
                <p className="mb-2 text-[10px] leading-relaxed text-zinc-500">
                  ...
                  to reposition, rotate with the top-right handle, resize with the bottom-right
                  handle, or use the trash icon to remove.
                </p>
                {selectedGraphic ? (
                ...
                <section className="mb-6 space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    {selectedGraphic ? (
                      <div className="rounded-lg border border-white/8 bg-zinc-900/40 px-3 py-2.5">
                        <div className="flex items-start gap-2">
                          <Move className="mt-0.5 h-3.5 w-3.5 shrink-0 text-sky-300/80" aria-hidden />
                          <p className="text-[10px] leading-relaxed text-zinc-500">
                            Drag the graphic to reposition it. Use the sky handle to rotate, the
                            purple corner to resize, or the size slider below — up to 400%.
                          </p>
                        </div>
                        <div className="mt-3">
                          <p className="mb-1.5 text-[10px] font-medium text-zinc-500">Rotation</p>
                          <div className="flex flex-wrap gap-1">
                            {mockupRotatePresets.map((preset) => (
                              <button
                                key={`graphic-${preset}`}
                                type="button"
                                onClick={() => applyGraphicRotate(preset)}
                                className={`rounded-md border px-2 py-1 text-[10px] tabular-nums transition ${
                                  currentGraphicRotate === preset
